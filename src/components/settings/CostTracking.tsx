...
export default function CostTracking() {
  ...
  const addCostItem = (period: 'weekly' | 'monthly' | 'annual') => {
    ...
    const vatAmount = cost.hasVat ? roundToCurrency(amount * 0.2) : 0;
  ...
  const calculateTotal = (items: CostItem[]) => {
    const total = items.reduce((total, item) => total + item.amount + (item.vatAmount || 0), 0);
  ...
  const calculateProjections = () => {
    ...
    // Use accurate 13.04 weeks per quarter (52 weeks ÷ 4 quarters = 13.04)
    return {
      quarterly: roundToCurrency((weeklyTotal * 13.04) + (monthlyTotal * 3) + (annualTotal / 4)),
      yearly: roundToCurrency((weeklyTotal * 52) + (monthlyTotal * 12) + annualTotal)
    };
  };
