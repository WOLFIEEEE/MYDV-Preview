...
export async function GET() {
  try {
    ...
    // Fetch all sale details for this dealer
    const allSaleDetails = await db
      .select()
      .from(saleDetails)
      .where(eq(saleDetails.dealerId, dealerId))
      .orderBy(desc(saleDetails.createdAt));

    return NextResponse.json({
      success: true,
      data: allSaleDetails,
      count: allSaleDetails.length
    });
